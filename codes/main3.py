...
class DriftDetiectionFramework():
    ...
    def __init__(self, outputFile = None):
        ...
        self.fieldnames = ["level", 'precision_rate', 'miss_label_rate', 'error_probability', 'standard_deviation', "min_error_probability", 'self.minStandardDeviation', 'self.currentMin']
        self.csvWriter = None
    ...
    def detect(self, predict_labels, real_labels, k):
        ...
        if (error_probability + standard_deviation) < self.currentMin:
        ...
        return {"level": l, 'precision_rate': precision_rate, 'miss_label_rate': miss_label_rate, 'error_probability': error_probability, 'standard_deviation': standard_deviation, "min_error_probability": self.minErrorProbability, 'self.minStandardDeviation': self.minStandardDeviation, 'self.currentMin': self.currentMin}


    def prepareClasasifier(self, trainingSet, partition = 0):
        trainingSet = [LabeledPoint(d[-1], d[:-1]) for d in trainingSet]
        self.classifier = NaiveBayes.train(sc.parallelize(trainingSet))
        return self
    ...
    def predict(self, dataset, split_size):
        ...
        for i, v in enumerate(split_trainingSet):
            ...
            if self.retrainStrategy:
                ...
                if len(trainset) > 0:
                    self.classifier = NaiveBayes.train(sc.parallelize(trainset))
                self.retrainStrategy = False
            ...
            y_pred = array(self.classifier.predict(sc.parallelize(predict_data)).collect())
